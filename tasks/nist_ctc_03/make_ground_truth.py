#!/usr/bin/env python3
"""
make_ground_truth.py — build the NIST CTC-03 ground truth from the provided STEP.

NIST PMI CTC-03 (Complementary Test Case 3) — AP242-e2 annotated part.
From: https://pages.nist.gov/CAD-PMI-Testing/  (US Government work — unrestricted).

Source STEP: nist_ctc_03_asme1_ap242-e2.stp  (~658 KB)
CTC parts are generally more complex than FTC; CTC-03 is expected to be a medium-hard
prismatic part with pockets, bosses, and possibly filleted/chamfered transitions.
...
def main():
    ...
    ap.add_argument("--src", default=None,
                    help="path to the CTC-03 STEP file (default: ground_truth/result.step)")
    args = ap.parse_args()
    ...
    if args.src:
        ...
        staged = HERE.parents[1] / "_staging" / "nist" / "nist_ctc_03_asme1_ap242-e2.stp"
        src = staged
    if not src.exists():
        raise SystemExit(
            f"CTC-03 STEP not found at {src}. Provide it via --src or place it at "
            f"{dst_step}.")
