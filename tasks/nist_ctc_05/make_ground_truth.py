#!/usr/bin/env python3
"""
make_ground_truth.py — build the NIST CTC-05 ground truth from the provided STEP.

NIST PMI CTC-05 (Complementary Test Case 5) — AP242-e1 annotated part.
From: https://pages.nist.gov/CAD-PMI-Testing/  (US Government work — unrestricted).

Source STEP: nist_ctc_05_asme1_ap242-e1.stp  (~857 KB)
CTC-05 is one of the more complex CTC parts; expected to have multiple prismatic
features, compound pockets, potential revolved features, and rich GD&T annotations.
...
def main():
    ...
    ap.add_argument("--src", default=None,
                    help="path to the CTC-05 STEP file (default: ground_truth/result.step)")
    args = ap.parse_args()
    ...
    if args.src:
        ...
        staged = HERE.parents[1] / "_staging" / "nist" / "nist_ctc_05_asme1_ap242-e1.stp"
        src = staged
    if not src.exists():
        raise SystemExit(
            f"CTC-05 STEP not found at {src}. Provide it via --src or place it at "
            f"{dst_step}.")
