#!/usr/bin/env python3
"""
make_ground_truth.py — build the NIST FTC-07 ground truth from the provided STEP.

NIST PMI FTC-07 (Functional Test Case 7) — AP242-e2 annotated part.
From: https://pages.nist.gov/CAD-PMI-Testing/  (US Government work — unrestricted).

Source STEP: nist_ftc_07_asme1_ap242-e2.stp
Expected: medium complexity prismatic part with holes, chamfers, GD&T annotations.
...
def main():
    ...
    ap.add_argument("--src", default=None,
                    help="path to the FTC-07 STEP file (default: ground_truth/result.step)")
    args = ap.parse_args()
    ...
    if args.src:
        ...
        staged = HERE.parents[1] / "_staging" / "nist" / "nist_ftc_07_asme1_ap242-e2.stp"
        src = staged
    if not src.exists():
        raise SystemExit(
            f"FTC-07 STEP not found at {src}. Provide it via --src or place it at "
            f"{dst_step}.")
