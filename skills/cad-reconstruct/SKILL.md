...
description: Reconstruct a mechanical part by WRITING a build123d program (a candidate.py defining a module-level `result` solid) and iterating it against a deterministic geometric grader — the propose → build → grade → revise loop. Use this whenever you need to PRODUCE CAD code from a written geometry spec, reproduce/match a reference STEP/STL, fix a build123d candidate that's scoring wrong (e.g. volume too high because a hole isn't subtracting), add missing fillets/holes to climb the topology score, or run the CAD autoresearch loop. Triggers on "model this part", "build123d", "match this STEP / STL", "reconstruct this part", "write the candidate", "my volume/topology is off, fix the candidate", or working inside a cad-autoresearch repo. If the input is a 2D drawing IMAGE, read it into a spec first with the drawing-read skill, then use this. To SCORE a finished STEP against a reference (consume a part, not produce one), use the cad-grade skill instead. Do not use for freeform generative/artistic 3D, mesh sculpting, or CAM/toolpathing.
triggers:
  - "model this part"
