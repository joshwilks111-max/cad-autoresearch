---
name: cad-grade
description: Score an existing CAD GEOMETRY part (a STEP file, or a finished build123d candidate) against a reference, deterministically, through the same multi-layer geometric referee the autoresearch loop uses. Use this whenever you already HAVE a part and want to know how close it is — certify a reconstruction's fidelity, compare a human STEP to an AI one on equal footing, or grade a finished part against a registered task's ground truth or a bring-your-own reference STEP. Triggers on "grade this STEP", "score this part against the reference", "how close is this to the target / is it solved", "is this reconstruction correct", "compare these two STEP files", or running the time-trial referee. Returns a composite plus per-layer scores (body/bbox/volume/IoU/topology/chamfer) and actionable gaps. This is for 3D CAD geometry ONLY — not grading essays, code, or anything non-geometric (the word "grade" alone is not enough). Do not use to model/produce a part (that's cad-reconstruct) or to read a 2D drawing image (that's drawing-read).
triggers:
  - "grade this STEP"
